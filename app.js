...
const BACKEND_DOMAIN = "beta-restaurante.onrender.com";
...
let RESTAURANT_ID = localStorage.getItem("session_rest_id") || sessionStorage.getItem("session_rest_id") || "";
...
function checkSession(requiredRole) {
    // Lee de localStorage (Mantener sesión) o sessionStorage (sesión temporal)
    const role = localStorage.getItem("session_role") || sessionStorage.getItem("session_role");
    const rest = localStorage.getItem("session_rest_id") || sessionStorage.getItem("session_rest_id");
    const token = localStorage.getItem("session_token") || sessionStorage.getItem("session_token");
    ...
    // Sincronizar RESTAURANT_ID global
    RESTAURANT_ID = rest;

    // Cajeros y managers pueden entrar al admin
    if (requiredRole === 'admin' && (role === 'cajero' || role === 'manager')) {
        return;
    }
...
function addAdminBackButton() {
    ...
    if (role === 'admin' || role === 'manager' || role === 'cajero') {
...
async function addProduct() {
    ...
    // Nuevos campos
    const skuElem = document.getElementById('p_sku');
    const costElem = document.getElementById('p_cost');
    const sku = skuElem ? skuElem.value : "";
    
    let costVal = costElem && costElem.value ? parseFloat(costElem.value) : 0;
    const cost = isNaN(costVal) ? 0 : costVal;
    
    if(!name || isNaN(price)) {
        alert("Completa los campos nombre y precio válido");
        return;
    }
    ...
    try {
        ...
        if(res.ok) {
            if(typeof window.showToast === 'function') window.showToast("Producto agregado");
            else alert("Producto agregado");
            
            document.getElementById('p_name').value = "";
            document.getElementById('p_price').value = "";
            if(document.getElementById('p_stock')) document.getElementById('p_stock').value = "";
            if(skuElem) skuElem.value = "";
            if(costElem) costElem.value = "";
            
            if(typeof window.loadMenuAdmin === 'function') window.loadMenuAdmin();
            else loadMenuAdmin();
            ...
            alert(`Error al agregar producto: ${err.detail || 'Verifica la conexión'}`);
        }
    } catch(e) { 
        console.error(e); 
...
async function addTable() {
    ...
    try {
        ...
        if(res.ok) {
            if(typeof window.showToast === 'function') window.showToast("Mesa agregada");
            else alert("Mesa agregada");
            
            document.getElementById('t_name').value = "";
            if(typeof window.loadTablesAdmin === 'function') window.loadTablesAdmin();
            else loadTablesAdmin();
        } else {
            const errData = await res.json().catch(() => ({}));
            alert(`Error al agregar mesa: ${errData.detail || 'Verifica la conexión'}`);
        }
    } catch(e) { 
        console.error(e); 
        alert("Error de red: " + e.message);
...
async function addStaff() {
    ...
    try {
        ...
        const data = await res.json();
        if(res.ok) {
            if(typeof window.showToast === 'function') window.showToast("Empleado agregado");
            else alert("Empleado agregado");
            
            document.getElementById('s_name').value = "";
            document.getElementById('s_user').value = "";
            document.getElementById('s_pwd').value = "";
            if(typeof window.loadStaffAdmin === 'function') window.loadStaffAdmin();
            else loadStaffAdmin();
            
            const modal = document.getElementById('modal-add-staff');
            if(modal) modal.classList.add('hidden');
        } else {
            alert(`Error: ${data.detail || 'Petición fallida'}`);
        }
    } catch(e) { 
        console.error(e); 
        alert("Error de red al contratar: " + e.message);
    }
}
...
async function openShift() {
    ...
    try {
        ...
        if(res.ok) {
            if(typeof window.showToast === 'function') window.showToast("Turno abierto exitosamente");
            else alert("Turno abierto exitosamente");
            document.getElementById('opening_balance').value = "";
            loadShiftAdmin();
        } else {
            const err = await res.json().catch(() => ({}));
            alert(`Error al abrir turno: ${err.detail || 'Verifica la conexión'}`);
        }
    } catch(e) {
        console.error(e);
...
async function closeShift() {
    ...
    try {
        ...
        if(res.ok) {
            if(typeof window.showToast === 'function') window.showToast("Turno cerrado correctamente (Corte Z)");
            else alert("Turno cerrado correctamente (Corte Z)");
...
async function sendOrder() {
    ...
    try {
        ...
        btn.innerText = "Enviar Orden a Cocina";
        btn.disabled = false;
    }
}
